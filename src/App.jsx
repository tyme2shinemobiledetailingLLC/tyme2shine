...
const TABS = ['Customers', 'Inventory', 'Time Clock', 'Calendar', 'Goals', 'Notes']
...
const s = {
  ...
  textarea: { padding: 10, border: `1px solid ${colors.blue}`, borderRadius: 6, width: '100%', marginBottom: 10, boxSizing: 'border-box', background: colors.input, color: colors.text, minHeight: 80, resize: 'vertical' },
  ...
  noteCard: { border: `1px solid ${colors.blue}`, borderRadius: 8, padding: 14, marginBottom: 10, background: colors.card },
}
...
export default function App() {
  ...
  const loadAll = async () => {
    const [c, i, j, a, g, n] = await Promise.all([
  ...
  const addNote = async () => {
    if (!noteAuthor || !noteMessage) return
    const { data, error } = await supabase.from('notes').insert([{ author: noteAuthor, message: noteMessage }]).select()
    if (error) { alert('Error: ' + error.message); return }
    if (data) setNotesList([...data, ...notesList])
    setNoteMessage('')
  }
  ...
  return (
    <div style={s.app}>
        ...
        <div>
          ...
          {clocks.map(c => (
            <div key={c.id} style={s.card}>
              ...
              {c.time_out
                ? <span style={{color: colors.green}}>Finished: {c.time_out}</span>
            ...
            <div key={period}>
              ...
              {goals.filter(g => g.period === period).map(g => (
                <div key={g.id} style={{...s.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                  <span style={{color: g.done ? colors.green : colors.text, textDecoration: g.done ? 'line-through' : 'none'}}>{g.text}</span>
                  <div style={{display: 'flex', gap: 8}}>
                    <button style={g.done ? {...s.btnBlue, background: colors.green, color: '#000'} : s.btnBlue} onClick={() => toggleGoal(g.id, g.done)}>
                      {g.done ? '✅' : 'Done'}
                    </button>
      ...
      {tab === 'Notes' && (
        <div>
          <h2 style={s.sectionTitle}>📝 Add Note</h2>
          <div style={s.label}>Your Name</div>
          <input style={s.input} placeholder="e.g. Tymere or Partner's name" value={noteAuthor} onChange={e => setNoteAuthor(e.target.value)} />
          <div style={s.label}>Note</div>
          <textarea style={s.textarea} placeholder="Write your note here..." value={noteMessage} onChange={e => setNoteMessage(e.target.value)} />
          <button style={s.btn} onClick={addNote}>Post Note</button>
          <h2 style={{...s.sectionTitle, marginTop: 24}}>💬 Notes ({notesList.length})</h2>
          {notesList.length === 0 && <p style={{color: colors.subtext}}>No notes yet.</p>}
          {notesList.map(n => (
            <div key={n.id} style={s.noteCard}>
              <strong style={{color: colors.accent}}>{n.author}</strong>
              <span style={{color: colors.subtext, fontSize: 12, marginLeft: 10}}>
                {new Date(n.created_at).toLocaleDateString()} {new Date(n.created_at).toLocaleTimeString([], {hour: '2-digit', minute: '2-digit'})}
              </span><br />
              <p style={{color: colors.text, margin: '8px 0'}}>{n.message}</p>
              <button style={s.btnRed} onClick={() => deleteNote(n.id)}>🗑 Remove</button>
            </div>
          ))}
        </div>
      )}
